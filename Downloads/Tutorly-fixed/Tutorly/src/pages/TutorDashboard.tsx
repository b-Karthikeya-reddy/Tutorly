...
export default function TutorDashboard() {
  ...
  const isSlotPast = (slotDate: string) => {
            ...
            <div className="rounded-2xl border border-border bg-white p-6 space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <textarea value={profile.bio} onChange={(e) => setProfile({ ...profile, bio: e.target.value })}
                  placeholder="Tell students about yourself..." rows={4}
              ...
              <div className="space-y-1.5">
                ...
                <textarea value={profile.qualifications} onChange={(e) => setProfile({ ...profile, qualifications: e.target.value })}
                  placeholder="Dean's List, Teaching Assistant..." rows={2}
                  className="w-full rounded-xl border border-input px-3 py-2.5 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 resize-none" />
              </div>
              <div className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {profile.subjects.map((s) => (
                    <span key={s} className="flex items-center gap-1.5 rounded-full bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
                      {s}
                      <button onClick={() => removeSubject(s)} className="hover:text-primary/60"><X className="h-3 w-3" /></button>
                    </span>
                  ))}
                </div>
              </div>
            ...
            <div className="rounded-2xl border border-border bg-white p-6">
              ...
              {reviews.length === 0 ? (
                <div className="flex flex-col items-center py-12 text-center">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Reviews appear after students complete sessions</p>
                </div>
              ) : (
